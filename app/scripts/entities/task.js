  ...
  App.module("Entities", function(Entities, App, Backbone, Marionette, $, _){
    ...
    Entities.Task = Backbone.Model.extend({
      ...
      sync: function(method, model, options) {
        ...
        switch (method) {
            case "create":
                options.url = "http://api.multikanban.com/users/"+App.loggedInUser.id+"/kanbans/"+this.kanban_id+"/tasks";
                break;
            case "read":
                options.url = "http://api.multikanban.com/users/"+App.loggedInUser.id+"/kanbans/"+this.kanban_id+"/tasks/"+model.get("id");
                break;
            case "delete":
                options.url = "http://api.multikanban.com/users/"+App.loggedInUser.id+"/kanbans/"+this.kanban_id+"/tasks/"+model.get("id");
                break;
    ...
    Entities.TaskCollection = Backbone.Collection.extend({
      ...
      initialize: function(collection, options){
        ...
        this.url = "http://api.multikanban.com/users/"+App.loggedInUser.id+"/kanbans/"+options.kanban_id+"/tasks/"+options.state;
      }
    });
