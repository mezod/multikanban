  ...
  App.module("Entities", function(Entities, App, Backbone, Marionette, $, _){
    ...
    Entities.Kanban = Backbone.Model.extend({
      ...
      sync: function(method, model, options) {
        ...
        switch (method) {
            case "create":
                options.url = "http://api.multikanban.com/users/"+App.loggedInUser.id+"/kanbans";
                break;
            case "read":
                options.url = "http://api.multikanban.com/users/"+App.loggedInUser.id+"/kanbans/"+model.get("id");
                break;
            case "delete":
                options.url = "http://api.multikanban.com/users/"+App.loggedInUser.id+"/kanbans/"+model.get("id");
                break;
    ...
    Entities.KanbanCollection = Backbone.Collection.extend({
      ...
      fetch: function(options){
        this.url = "http://api.multikanban.com/users/"+App.loggedInUser.id+"/kanbans";
